...
interface FloatingIcon { svgBody: string; slug: string; }

interface Props { floatingIcons: FloatingIcon[]; }

// Fixed layout slots for scattered icons — avoids hydration mismatch
const SLOTS = [
  { top: "6%",  left: "14%", size: 58, delay: 0,   dur: 5.4, bg: "bg-blue-100   dark:bg-blue-900/40"   },
  { top: "14%", left: "64%", size: 50, delay: 1.1, dur: 4.9, bg: "bg-purple-100 dark:bg-purple-900/40" },
  { top: "26%", left: "76%", size: 66, delay: 0.5, dur: 6.1, bg: "bg-pink-100   dark:bg-pink-900/40"   },
  { top: "35%", left: "20%", size: 46, delay: 1.7, dur: 5.0, bg: "bg-green-100  dark:bg-green-900/40"  },
  { top: "46%", left: "52%", size: 62, delay: 0.3, dur: 5.7, bg: "bg-amber-100  dark:bg-amber-900/40"  },
  { top: "55%", left: "8%",  size: 54, delay: 2.0, dur: 4.6, bg: "bg-orange-100 dark:bg-orange-900/40" },
  { top: "60%", left: "70%", size: 50, delay: 0.8, dur: 6.0, bg: "bg-teal-100   dark:bg-teal-900/40"   },
  { top: "71%", left: "36%", size: 58, delay: 1.4, dur: 5.3, bg: "bg-indigo-100 dark:bg-indigo-900/40" },
  { top: "80%", left: "16%", size: 44, delay: 0.6, dur: 4.8, bg: "bg-rose-100   dark:bg-rose-900/40"   },
  { top: "20%", left: "40%", size: 54, delay: 1.2, dur: 5.6, bg: "bg-cyan-100   dark:bg-cyan-900/40"   },
  { top: "83%", left: "60%", size: 64, delay: 2.2, dur: 5.2, bg: "bg-violet-100 dark:bg-violet-900/40" },
  { top: "48%", left: "84%", size: 46, delay: 0.4, dur: 6.3, bg: "bg-emerald-100 dark:bg-emerald-900/40" },
];

export default function LoginPageClient({ floatingIcons }: Props) {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      {/* ── Left panel — floating icons ────────────────────────────── */}
      <div className="hidden lg:flex lg:w-[45%] relative bg-ink-50 dark:bg-ink-900 overflow-hidden flex-col justify-end p-12">
        {floatingIcons.slice(0, SLOTS.length).map((icon, i) => {
          const slot = SLOTS[i];
          return (
            <div
              key={icon.slug}
              className={`absolute rounded-2xl ${slot.bg} flex items-center justify-center opacity-70`}
              style={{
                top: slot.top,
                left: slot.left,
                width: slot.size,
                height: slot.size,
                animation: `digche-float ${slot.dur}s ease-in-out ${slot.delay}s infinite`,
              }}
            >
              <div
                className="text-ink-600 dark:text-ink-300"
                style={{ width: slot.size * 0.5, height: slot.size * 0.5 }}
        ...
        <div className="relative z-10">
          <p className="text-2xl font-bold text-ink-900 dark:text-white tracking-tight">
            Digche Icons
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 bg-white dark:bg-ink-950 min-h-screen">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8 text-center lg:text-left">
            <span className="text-xl font-bold text-ink-900 dark:text-white tracking-tight">Digche</span>
          </div>
          ...
          <p className="mt-6 text-xs text-center text-ink-400 dark:text-ink-500">
            ...
            <span className="underline cursor-pointer hover:text-ink-700 dark:hover:text-ink-300">terms of service</span>.
          </p>
        </div>
      </div>
    </div>
