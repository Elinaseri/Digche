...
export default function EditIconForm({ icon }: Props) {
  ...
  function handleVariantChange(style: IconStyle, content: string | null) {
    if (content === null) {
      const existing = existingVariants[style];
      if (!existing) return;
      if (!confirm("Remove this variant?")) return;
      startTransition(async () => {
        await removeVariantAction(existing.id, icon.id);
        setLocalPreviews((prev) => ({ ...prev, [style]: null }));
        router.refresh();
      });
    } else {
      const fd = new FormData();
      fd.set(`variant_${style}`, content);
      startTransition(async () => {
        const res = await addVariantAction(icon.id, icon.slug, fd);
        if (!res.error) {
          setLocalPreviews((prev) => ({ ...prev, [style]: content }));
          router.refresh();
        }
      });
    }
  ...
  function handleSaveMeta(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.error) { setMetaError(res.error); }
      else { setMetaSaved(true); router.refresh(); }
    });
  }
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      >
        <h2 className="text-sm font-semibold text-ink-900 dark:text-white">Metadata</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-ink-700 dark:text-ink-300">
              Slug <span className="text-ink-400 dark:text-ink-500 font-normal ml-1">read-only</span>
            </label>
            <input value={icon.slug} readOnly
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-ink-700 dark:text-ink-300">
              Tags <span className="text-ink-400 dark:text-ink-500 font-normal ml-1">comma-separated</span>
            </label>
        ...
        {metaError && (
          <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2">{metaError}</p>
        )}
        {metaSaved && (
          <p className="text-sm text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg px-3 py-2">Saved.</p>
        )}
      ...
      {/* SVG Variants + Code Editor — side by side, matched height */}
      <div className="flex flex-col lg:flex-row lg:items-stretch gap-8">

        {/* Variants card */}
        <div className="flex-1 min-w-0 max-w-2xl bg-white dark:bg-ink-800 border border-ink-200 dark:border-ink-700 rounded-2xl p-6 space-y-5">
          <div>
            ...
            <p className="text-xs text-ink-400 dark:text-ink-500 mt-0.5">
              Click a tile to replace. Use the &lt;/&gt; button to edit code directly.
            </p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {STYLES.map((style) => (
              <SvgVariantSlot
                key={style}
                style={style}
                svgContent={getTileContent(style)}
                onChange={(content) => handleVariantChange(style, content)}
                isSelected={activeStyle === style}
                onSelect={() => openEditor(style)}
              />
            ))}
          </div>
        </div>

        {/* Code editor card — same height as Variants card */}
        {activeStyle && (
          <div className="lg:w-80 xl:w-96 flex-shrink-0 flex flex-col">
            <div className="flex-1 bg-white dark:bg-ink-800 border border-ink-200 dark:border-ink-700 rounded-2xl p-6 flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setActiveStyle(null)}
              ...
              <textarea
                ...
                className="flex-1 min-h-[160px] p-3 rounded-xl border border-ink-200 dark:border-ink-700 bg-ink-50 dark:bg-ink-900 text-xs font-mono text-ink-800 dark:text-ink-200 resize-none focus:outline-none focus:border-ink-400 dark:focus:border-ink-500 leading-relaxed"
              ...
              <button type="button" onClick={handleApplyCode} disabled={pending}
